...
fn get_coef_cor(x: &Vec<f32>, y: &Vec<f32>) -> f32 {
    ...
    let n = x.len() as f32;
    let mean_x = x.iter().map(|&xi| xi).sum::<f32>() / n;
    let mean_y = y.iter().map(|&yi| yi).sum::<f32>() / n;
    ...
    // Дисперсии
    let std_x = (x.iter().map(|&xi| (xi - mean_x).powi(2)).sum::<f32>() / (n - 1.0))
        .sqrt();
    let std_y = (y.iter().map(|&yi| (yi - mean_y).powi(2)).sum::<f32>() / (n - 1.0))
        .sqrt();
