...
impl Fibr {
    pub fn new(coef_fibr: &Vec<f32>, trs: &Vec<f32>, mask: &Vec<usize>) -> Fibr {
        let mut fibr = Fibr {
            start_ind_arr: vec![],
            stop_ind_arr: vec![],
            diff_stop_start: vec![],
        };
        fibr.get_start_stop(coef_fibr, trs, mask);
        fibr
    }

    fn get_start_stop(&mut self, coef_fibr: &Vec<f32>, trs: &Vec<f32>, mask: &Vec<usize>) {
        let mut start_ind: usize = 0;
        let mut stop_ind: usize = 0;
        let min_size: usize = 22;
        let mut flag: bool = false;
        if coef_fibr[0] > trs[0] {
            self.start_ind_arr.push(start_ind);
            flag = true;
        }
        for i in 0..coef_fibr.len() - 1 {
            if (flag == false) && (coef_fibr[i] < trs[i]) && (coef_fibr[i + 1] > trs[i + 1]) && (mask[i] == 1) {
                ...
                if i - start_ind > min_size {
                    stop_ind = i;
                    self.stop_ind_arr.push(stop_ind);
                } else {
